...
class Conv_LSTM(nn.Module):
    def __init__(self, in_features, hidden_features, num_classes, num_channels=1,batch_first=False, bidirectional=False):
        super(Conv_LSTM, self).__init__()
        self.block1 = ConvBlock(num_channels, num_channels*4, kernel=[2,6])
    ...
    def forward(self,x):
        try:
          ...
          print(x.size())
        # x = x.view(x_batch, 1, 5, -1)
        x = self.block1(x)
        x = x.view(x_batch,5,-1)
        x, [h,c]= self.lstm(x)
        x = self.relu(x)
        x = self.fc1(x)
        x = x[:, -1]
